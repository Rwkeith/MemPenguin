...
void ConsoleWindow::Draw(bool &pOpen)
{
    ...
    if (!ImGui::Begin("MemPenguin Main Console", &pOpen))
    {
        ImGui::End();
        return;
    }
...
void ConsoleWindow::ExecCommand(const char* command_line)
{
    ...
    else if(Stricmp(command_line, "processes") == 0)
    {
        if (!mainInterface->showProcWindow)
        {
            AddLog("Opening process window...");
            mainInterface->showProcWindow = true;
        }
    }
    else
    {
...
void ConsoleWindow::RegisterWindow(MainInterface* mainUI)
{
